...
<template>
  <main class="app-page app-home-page">
    <section v-if="enteringName && inviteMode"
      class="app-screen app-home-container app-lobby app-lobby-invite-only">
      ...
      <form class="app-lobby-create-form" @submit.prevent="submit">
        <template v-if="reconnectMode">
          ...
          <select id="reconnect-player-select" v-model="selectedReconnectPlayerId"
            class="app-text-input" :disabled="lobby.reconnectPending">
            <option v-for="player in reconnectPlayers" :key="player.id" :value="player.id">{{
              player.name }}</option>
          </select>
          <p v-if="lobby.reconnectPending" class="app-subtitle" role="status">Demande envoyée au
            Maître du Jeu. En attente de validation…</p>
        </template>
        <template v-else>
          <label for="player-name-input">Votre nom</label>
          <AppInput id="player-name-input" v-model="playerName" maxlength="20" autocomplete="off"
            placeholder="Votre nom..." class="app-text-input" autofocus />
        </template>
    ...
    <section v-else-if="lobby.hasStoredSession && !lobby.hasSession"
      class="app-screen app-home-container" aria-live="polite">
      ...
      <div class="app-button-group">
        <AppButton variant="primary" :disabled="lobby.restoringSession"
    ...
    <section v-else-if="enteringName" class="app-screen app-home-container app-lobby">
      ...
      <div v-if="inviteLobbyId" class="app-lobby-invite-hint">
        ...
        <AppButton variant="primary" :disabled="!canSubmit || joiningLobbyId !== null"
          :loading="joiningLobbyId !== null" @click="joinInviteLobby">
    ...
    <section v-else class="app-home-shell">
      ...
      <nav class="app-home-actions" aria-label="Actions principales">
        <a v-if="!staticMode" id="lobbies-btn" :href="appPath(ROUTE_PATH.LOBBIES)"
          class="app-home-action app-home-action-primary">
        ...
        <a :href="PUBLIC_LINK.WIKI" class="app-home-action" target="_blank"
