...
class FakeGateway implements LobbyGateway {
  ...
  readonly getReconnectOptions = vi.fn(async (_lobbyId: string): Promise<Ack<LobbySnapshot>> => ackSuccess(createLobby()))
  readonly requestReconnect = vi.fn(async (_lobbyId: string, _playerId: PlayerId): Promise<Ack<EmptyResponse>> => ackSuccess({}))
  readonly approveReconnect = vi.fn(async (_requestId: string): Promise<Ack<EmptyResponse>> => ackSuccess({}))
  readonly rejectReconnect = vi.fn(async (_requestId: string): Promise<Ack<EmptyResponse>> => ackSuccess({}))
